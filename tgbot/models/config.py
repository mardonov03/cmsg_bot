...
class GroupModel(MainModel):
    ...
    async def get_creator(self, chat_id) -> int:
        try:
            async with self.pool.acquire() as conn:
                creator = await conn.fetchval('SELECT creator FROM groups WHERE groupid = $1', chat_id)
                return creator
        except Exception as e:
            logging.error(f'get_creator error: {e}')
